...
#else
        ESP_RETURN_ON_FALSE(false, NULL, TAG, "MIPI-DSI interface is not supported (by chip or IDF < 5.3)!");
#endif
    }
    lvgl_port_unlock();
...
#else
        ESP_RETURN_ON_FALSE(false, NULL, TAG, "RGB interface is not supported (by chip or IDF < 5.0)!");
#endif
...
#elif (SOC_MIPI_DSI_SUPPORTED && ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 3, 0))
